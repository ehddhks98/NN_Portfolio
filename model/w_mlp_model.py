...
class AdaptivePortfolioOptimizer(nn.Module):
    def __init__(self, num_assets=5, hidden_size=64, num_channels=[32, 64, 128], kernel_size=3, dropout=0.2):
        ...
        # 가중치 예측기 (MLP) - 입력 차원 수정
        # 입력: 기대수익률 + 공분산 행렬의 대각선 요소들
        input_dim = 1 + num_assets  # 기대수익률(num_assets) + 공분산 대각선(num_assets)
    ...
    def forward(self, asset_data, common_data, future_returns):
        ...
        # CAPM을 사용한 기대 수익률 계산 (common_data에서 직접 추출)
        ...
        """
        input: expected returns, cov matrix
        output: weights
        model: MLP
        expected returns: (batch_size, num_assets)-> (batch_size, num_assets, 1)
        cov matrix: (batch_size, num_assets, num_assets)
        """
        mu_expanded = expected_returns.unsqueeze(-1)
        predictor_input = torch.cat([mu_expanded, cov_matrix], dim=2)
        predictor_input = predictor_input.view(-1, predictor_input.size(-1))
        logits = self.weight_predictor(predictor_input)
        logits = logits.view(betas.shape[0], self.num_assets)
        weights_sum = logits.sum(dim=1, keepdim=True)
        weights = logits / weights_sum
