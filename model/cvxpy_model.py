...
class BetaEstimator(nn.Module):
    ...
    def forward(self, asset_data, common_data):
        ...
        # 새로운 베타 예측 파이프라인
        hidden1_out = self.beta_hidden1(combined_flat)  # (batch*assets, hidden_size)
        hidden2_out = self.beta_hidden2(hidden1_out)    # (batch*assets, hidden_size//2)
        betas_raw = self.beta_output(hidden2_out)       # (batch*assets, 1)
        
        # 극단적인 베타 값만 제한 (자연스러운 범위 유지)
        # 베타는 일반적으로 -1 ~ 3 범위이지만, 극단적인 값만 방지
        betas_clamped = torch.clamp(betas_raw, min=-3.0, max=5.0)
        
        # 최종 형태로 재구성
        betas = betas_clamped.view(batch_size, num_assets)  # (batch, assets)
